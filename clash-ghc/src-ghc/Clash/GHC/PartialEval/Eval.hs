    ...
    Just f ->
      f eval pr args `catches`
        [ -- Catch an Eval specific error and attempt to correct it.
          ...
          --
          -- TODO These aren't caught, the primitives which can throw pure
          -- exceptions like this should use try in their implementation.
    ...
    VLam i x env
      ...
      | otherwise ->
          setLocalEnv env $ do
            ...
            -- traceM ("apply: copying " <> showPpr i <> " as " <> showPpr j)
